...
struct NegativeDistanceComputer : DistanceComputer {
        ...
        void LVQHNSW_add_vertices(
                IndexLVQHNSW& index_LVQHNSW,
                size_t n0,
                size_t n,
                const float* x,
                bool verbose,
                ...
                int max_level=0) {
            size_t d = index_LVQHNSW.d;
            HNSW& LVQHNSW = index_LVQHNSW.LVQHNSW;
            size_t ntotal = n0 + n;
            double t0 = getmillisecs();
            if (verbose) {
                printf("LVQHNSW_add_vertices: adding %zd elements on top of %zd "
                       "(preset_levels=%d)\n",
                       n,
                       n0,
                       int(preset_levels));
            ...
            //int max_level = LVQHNSW.prepare_level_tab(n, preset_levels);
            ...
            { // make buckets with vectors of the same level
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    int pt_level = LVQHNSW.levels[pt_id] - 1;
            ...
            idx_t check_period = InterruptCallback::get_period_hint(
                    max_level * index_LVQHNSW.d * LVQHNSW.efConstruction);
...
#pragma omp parallel if (i1 > i0 + 100)
                    {
                        ...
                        std::unique_ptr<DistanceComputer> dis(
                                storage_distance_computer(index_LVQHNSW.storage));
                        int prev_display =
...
#pragma omp for schedule(static)
                        for (int i = i0; i < i1; i++) {
                            ...
                            LVQHNSW.add_with_locks(*dis, pt_level, pt_id, locks, vt);
...
/**************************************************************
 * IndexLVQHNSW implementation
 **************************************************************/

    IndexLVQHNSW::IndexLVQHNSW(int d, int M, MetricType metric)
            : Index(d, metric), LVQHNSW(M) {}

    IndexLVQHNSW::IndexLVQHNSW(Index* storage, int M)
            : Index(storage->d, storage->metric_type), LVQHNSW(M), storage(storage) {}

    IndexLVQHNSW::~IndexLVQHNSW() {
        if (own_fields) {
            delete storage;
        }
    }

    void IndexLVQHNSW::train(idx_t n, const float* x) {
        FAISS_THROW_IF_NOT_MSG(
                storage,
                "Please use IndexLVQHNSWFlat (or variants) instead of IndexLVQHNSW directly");
        // LVQHNSW structure does not require training
        storage->train(n, x);
        is_trained = true;
    }
    std::vector<idx_t> IndexLVQHNSW::search_arrays(idx_t n, const std::vector<float> x, idx_t k, int param){
    ...
    void IndexLVQHNSW::search(
        ...
        idx_t check_period =
                InterruptCallback::get_period_hint(LVQHNSW.max_level * d * efSearch);
...
//#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder) schedule(guided)
                for (idx_t i = i0; i < i1; i++) {
                    ...
                    HNSWStats stats = LVQHNSW.search(*dis, k, idxi, simi, vt, params);
                    n1 += stats.n1;
                    n2 += stats.n2;
                    n3 += stats.n3;
                    ndis += stats.ndis;
        ...
        LVQHNSW_stats.combine({n1, n2, n3, ndis, nreorder});
    }

    void IndexLVQHNSW::add(idx_t n, const float* x) {
        FAISS_THROW_IF_NOT_MSG(
                storage,
                "Please use IndexLVQHNSWFlat (or variants) instead of IndexLVQHNSW directly");
    ...
    void IndexLVQHNSW::reconstruct(idx_t key, float* recons) const {
        storage->reconstruct(key, recons);
    }

    void IndexLVQHNSW::shrink_level_0_neighbors(int new_size) {
#pragma omp parallel
        {
            std::unique_ptr<DistanceComputer> dis(
                    storage_distance_computer(storage));
...
#pragma omp for
            for (idx_t i = 0; i < ntotal; i++) {
                size_t begin, end;
                LVQHNSW.neighbor_range(i, 0, &begin, &end);

                std::priority_queue<NodeDistFarther> initial_list;

                for (size_t j = begin; j < end; j++) {
                    int v1 = LVQHNSW.neighbors[j];
                    if (v1 < 0)
                        break;
                ...
                for (size_t j = begin; j < end; j++) {
                    if (j - begin < shrunk_list.size())
                        LVQHNSW.neighbors[j] = shrunk_list[j - begin].id;
                    else
                        LVQHNSW.neighbors[j] = -1;
                }
            }
        }
    }

    void IndexLVQHNSW::search_level_0(
            idx_t n,
            const float* x,
            idx_t k,
        ...
        storage_idx_t ntotal = LVQHNSW.levels.size();
...
#pragma omp critical
            { LVQHNSW_stats.combine(search_stats); }
        }
    }

    void IndexLVQHNSW::init_level_0_from_knngraph(
            int k,
            const float* D,
            const idx_t* I) {
        int dest_size = LVQHNSW.nb_neighbors(0);
...
#pragma omp parallel for
    ...
    void IndexLVQHNSW::init_level_0_from_entry_points(
            int n,
...
#pragma omp for schedule(dynamic)
            for (int i = 0; i < n; i++) {
                ...
                LVQHNSW.add_links_starting_from(
    ...
    void IndexLVQHNSW::reorder_links() {
        int M = LVQHNSW.nb_neighbors(0);
...
#pragma omp for
            for (storage_idx_t i = 0; i < ntotal; i++) {
                ...
                for (size_t j = begin; j < end; j++) {
                    storage_idx_t nj = LVQHNSW.neighbors[j];
                    if (nj < 0) {
                        end = j;
                        break;
                    }
                ...
                for (size_t j = begin; j < end; j++) {
                    LVQHNSW.neighbors[j] = tmp[order[j - begin]];
                }
            }
        }
    }

    void IndexLVQHNSW::link_singletons() {
        ...
        for (size_t i = 0; i < ntotal; i++) {
            ...
            LVQHNSW.neighbor_range(i, 0, &begin, &end);
            for (size_t j = begin; j < end; j++) {
                storage_idx_t ni = LVQHNSW.neighbors[j];
                if (ni >= 0)
                    seen[ni] = true;
            }
        }
    ...
    void IndexLVQHNSW::permute_entries(const idx_t* perm) {
 ...
 **************************************************************/
    ...
    ReconstructFromNeighborsLVQ::ReconstructFromNeighborsLVQ(
            const IndexLVQHNSW& index,
            size_t k,
            size_t nsq)
            : index(index), k(k), nsq(nsq) {
        M = index.LVQHNSW.nb_neighbors(0);
        FAISS_ASSERT(k <= 256);
        code_size = k == 1 ? 0 : nsq;
        ntotal = 0;
        d = index.d;
    ...
    void ReconstructFromNeighborsLVQ::reconstruct(
            storage_idx_t i,
            float* x,
            float* tmp) const {
        const HNSW& LVQHNSW = index.LVQHNSW;
        size_t begin, end;
        LVQHNSW.neighbor_range(i, 0, &begin, &end);
        ...
        if (k == 1 || nsq == 1) {
            ...
            for (size_t j = begin; j < end; j++) {
                storage_idx_t ji = LVQHNSW.neighbors[j];
                if (ji < 0)
                    ji = i;
    ...
    void ReconstructFromNeighborsLVQ::reconstruct_n(
            storage_idx_t n0,
            storage_idx_t ni,
            float* x) const {
#pragma omp parallel
...
#pragma omp for
    ...
    size_t ReconstructFromNeighborsLVQ::compute_distances(
            size_t n,
    ...
    void ReconstructFromNeighborsLVQ::get_neighbor_table(storage_idx_t i, float* tmp1)
    const {
        const HNSW& LVQHNSW = index.LVQHNSW;
        size_t begin, end;
        LVQHNSW.neighbor_range(i, 0, &begin, &end);
...
/// called by add_codes
    void ReconstructFromNeighborsLVQ::estimate_code(
            const float* x,
            storage_idx_t i,
            uint8_t* code) const {
    ...
    void ReconstructFromNeighborsLVQ::add_codes(size_t n, const float* x) {
...
/**************************************************************
 * IndexLVQHNSWFlat implementation
 **************************************************************/

    IndexLVQHNSWFlat::IndexLVQHNSWFlat() {
        is_trained = true;
    }
...
/**************************************************************
 * IndexLVQHNSWPQ implementation
 **************************************************************/

    IndexLVQHNSWPQ::IndexLVQHNSWPQ() = default;

    IndexLVQHNSWPQ::IndexLVQHNSWPQ(int d, int pq_m, int M, int pq_nbits)
            : IndexLVQHNSW(new IndexPQ(d, pq_m, pq_nbits), M) {
        own_fields = true;
        is_trained = false;
    }

    void IndexLVQHNSWPQ::train(idx_t n, const float* x) {
        IndexLVQHNSW::train(n, x);
        (dynamic_cast<IndexPQ*>(storage))->pq.compute_sdc_table();
    }

/**************************************************************
 * IndexLVQHNSWSQ implementation
 **************************************************************/

    IndexLVQHNSWSQ::IndexLVQHNSWSQ(
            int d,
            ScalarQuantizer::QuantizerType qtype,
            int M,
            MetricType metric)
            : IndexLVQHNSW(new IndexScalarQuantizer(d, qtype, metric), M) {
        is_trained = this->storage->is_trained;
        own_fields = true;
    }

    IndexLVQHNSWSQ::IndexLVQHNSWSQ() = default;

/**************************************************************
 * IndexLVQHNSW2Level implementation
 **************************************************************/

    IndexLVQHNSW2Level::IndexLVQHNSW2Level(
            Index* quantizer,
            size_t nlist,
            int m_pq,
            int M)
            : IndexLVQHNSW(new Index2Layer(quantizer, nlist, m_pq), M) {
        own_fields = true;
        is_trained = false;
    }

    IndexLVQHNSW2Level::IndexLVQHNSW2Level() = default;
...
// visno + 1 -> in result list + in candidates
        int search_from_candidates_2(
            ...
            while (candidates.size() > 0) {
                ...
                size_t begin, end;
                LVQHNSW.neighbor_range(v0, level, &begin, &end);
    ...
    void IndexLVQHNSW2Level::search(
            idx_t n,
            const float* x,
            idx_t k,
            float* distances,
        ...
        if (dynamic_cast<const Index2Layer*>(storage)) {
            IndexLVQHNSW::search(n, x, k, distances, labels);
...
#pragma omp parallel
            {
                ...
                int candidates_size = LVQHNSW.upper_beam;
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
                for (idx_t i = 0; i < n; i++) {
                    ...
                    for (int j = 0; j < LVQHNSW.upper_beam && j < k; j++) {
                        if (idxi[j] < 0)
                            break;
    ...
    void IndexLVQHNSW2Level::flip_to_ivf() {
