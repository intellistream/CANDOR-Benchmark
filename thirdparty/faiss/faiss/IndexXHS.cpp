...
IndexXHS::IndexXHS(
    ...
    replace_invlists(new BlockInvertedListsXHS(nlist, get_CodePacker(M, nbits_per_idx)), true);
...
CodePacker* IndexXHS::get_CodePacker(size_t M, size_t nbits_per_idx) {
    return new CodePackerXHS(M, nbits_per_idx);
}
/****************************************************************
 * training                                                     */

std::vector<idx_t> IndexXHS::search_arrays(idx_t n, const std::vector<float> x, idx_t k, int param){
        std::vector<float> distances(n*k);
        std::vector<idx_t> labels(n*k);
        XHSSearchParameters params_in;
        params_in.nprobe = param;
        search(n, x.data(), k, distances.data(), labels.data(), &params_in);
        return labels;
}

void IndexXHS::train_encoder(idx_t n, const float* x, const idx_t* assign) {
...
idx_t IndexXHS::train_encoder_num_vectors() const {
...
void IndexXHS::encode(idx_t key, const float* x, uint8_t* code) const {
    if (by_residual) {
...
void IndexXHS::sa_decode(idx_t n, const uint8_t* codes, float* x) const {
    size_t coarse_size = coarse_code_size();
...
int index_xhs_add_core_o_bs = 32768;

void IndexXHS::add_core_o(
        idx_t n,
        const float* x,
        const idx_t* xids,
        float* residuals_2,
        const idx_t* precomputed_idx) {
    idx_t bs = index_xhs_add_core_o_bs;
    if (n > bs) {
...
void IndexXHS::reconstruct_from_offset(
        int64_t list_no,
        int64_t offset,
        float* recons) const {
...
void IndexXHS::precompute_table() {
...
struct QueryTables {
    ...
    explicit QueryTables(
        ...
        if (auto ivfpq_params =
                    dynamic_cast<const XHSSearchParameters*>(params)) {
...
struct IVFPQScannerT : QueryTables {
    ...
    IVFPQScannerT(const IndexXHS& ivfpq, const IVFSearchParameters* params)
...
InvertedListScanner* IndexXHS::get_InvertedListScanner(
        bool store_pairs,
...
size_t IndexXHS::find_duplicates(idx_t* dup_ids, size_t* lims) const {
    size_t ngroup = 0;
    lims[0] = 0;
