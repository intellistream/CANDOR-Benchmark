...
struct XHSSearchParameters : IVFSearchParameters {
    size_t scan_table_threshold; ///< use table computation or on-the-fly?
    int polysemous_ht;           ///< Hamming thresh for polysemous filtering
    XHSSearchParameters() : scan_table_threshold(0), polysemous_ht(0) {}
    ~XHSSearchParameters() {}
};
...
struct IndexXHS : IndexIVF {
    ...
    CodePacker* get_CodePacker(size_t M, size_t nbits_per_idx);

    IndexXHS();
};

// block size used in IndexIVFPQ::add_core_o
FAISS_API extern int index_xhs_add_core_o_bs;
